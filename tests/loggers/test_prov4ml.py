...
def test_create_destroy_logger_context(logger_instance):
    ...
    with patch("itwinai.loggers.prov4ml.start_run") as start_run, patch("itwinai.loggers.prov4ml.end_run") as end_run:
...
def test_create_logger_context_should_log(logger_instance):
    logger_instance.should_log = MagicMock(return_value=True)
    logger_instance.create_logger_context(rank=1)
    with patch("itwinai.loggers.prov4ml.log_metric") as log_metric:
...
def test_save_hyperparameters(logger_instance):
    ...
    with patch("itwinai.loggers.prov4ml.log_param") as log_param:
...
def test_log_flops_per_batch(logger_instance):
    ...
    with patch("itwinai.loggers.prov4ml.log_flops_per_batch") as log_flops_pb:
        logger_instance.log(
...
def test_log_flops_per_epoch(logger_instance):
    ...
    with patch("itwinai.loggers.prov4ml.log_flops_per_epoch") as log_flops_pe:
        logger_instance.log(
...
def test_log_system(logger_instance):
    ...
    with patch("itwinai.loggers.prov4ml.log_system_metrics") as log_system:
...
def test_log_carbon(logger_instance):
    ...
    with patch("itwinai.loggers.prov4ml.log_carbon_metrics") as log_carbon:
...
def test_log_execution_time(logger_instance):
    ...
    with patch("itwinai.loggers.prov4ml.log_current_execution_time") as log_exec_time:
        logger_instance.log(
...
def test_log_model(logger_instance):
    ...
    with patch("itwinai.loggers.prov4ml.save_model_version") as log_model:
...
def test_log_best_model(logger_instance):
    ...
    model_mock = MagicMock()
    with patch("itwinai.loggers.prov4ml.log_model") as log_best_model:
        logger_instance.log(
...
def test_log_prov_documents(logger_instance, mlflow_run):
    ...
    with patch("itwinai.loggers.prov4ml.log_provenance_documents") as log_prov_documents:
